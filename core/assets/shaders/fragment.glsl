...
void main() {
	...
	finalColor += vec3((u_light0_intensity * v_color * u_light0_color.rgb) / pow(distance(v_position_world, u_light0_position), 1));
	finalColor += vec3((u_light1_intensity * v_color * u_light1_color.rgb) / pow(distance(v_position_world, u_light1_position), 1));
	finalColor += vec3((u_light2_intensity * v_color * u_light2_color.rgb) / pow(distance(v_position_world, u_light2_position), 1));
    gl_FragColor = finalColor; //vec4(v_textureCoordinate, 1.0, 1.0);
}
